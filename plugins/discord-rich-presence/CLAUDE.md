...
The host hands the client colours but no metrics, so `src/client/theme.client.ts` restates paseo's spacing, type, and radius scale, and fills in only the shades above `surface2` that the host keeps to itself.
