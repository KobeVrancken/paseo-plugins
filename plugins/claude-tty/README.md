...
**Projects on this host** appears only where the host has a `box-project` of its own — a machine that runs its sessions inside per-checkout containers, where being configured is what makes a checkout workable at all. It marks every Paseo project with what a session in it would get: a box, and the credentials its boundary lends; the host, because an allowlist says so, shown as `host:`; or nothing, which means a session there is refused before it starts. **Check** re-runs that project's validations and shows each one. It never writes — onboarding a project pauses for a token paste and deleting one throws away a checkout, so `box-project new` and `box-project delete` stay commands on the host.
