...
A pnpm workspace of paseo plugins, one per directory under `packages/`.
Each package is a plugin directory in its own right: paseo is installed against `packages/<name>`, and `index.ts` and `paseo-plugin.json` sit at that package's root because the loader stats exactly those names.
