...
Claude runs a status line command through a shell and renders its stdout, so the command is `cat > <runtime directory>/context.json`: one `cat`, no output, nothing on the terminal screen the adapter reads for readiness.
